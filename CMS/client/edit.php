...
<body>
    <div class="container">
        <h1 class="text-center text-dark my-4">Edit Account </h1>
        <form method="post" class="form-control">
            <div class="mb-3">
                <label for="name" class="form-label">Name:</label>
            <input type="text" name="name" id="Name" class="form-control" value="<?php echo $val['Name'] ?>">
            </div>
            <div class="mb-3">
                <label for="email" class="form-label">Emai:</label>
            <input type="email" name="email" id="email" class="form-control" value="<?php  echo $val['Email'] ?>">
            </div>
            <div class="mb-3">
                <label for="phone" class="form-label">Phone Number:</label>
            <input type="number" name="Phone" id="phone" class="form-control" value="<?php echo $val['Number'] ?>">
            </div>
            <div class="mb-3">
                <label for="marks" class="form-label">12<sup>th</sup> Marks:</label>
            <input type="number" name="marks" id="marks" class="form-control" value="<?php echo $val['Marks'] ?>">
            </div>
            <div class="mb-3">
                <label for="pass" class="form-label">Password:</label>
            <input type="password" name="pass" id="pass" class="form-control" value="<?php echo $val['Password'] ?>">
            </div>
            <div class="mb-3">
                <label for="c_pass" class="form-label">Confirm Password:</label>
            <input type="password" name="c_pass" id="c_pass" class="form-control" value="<?php echo $val['Password'] ?>">
            </div>
            <div class="mb-3">
            ...
            <input type="date" name="date" id="date" class="form-control" value="<?php echo $val['Date_Of_Birth'] ?>">
            </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name_err) && empty($email_err) && empty($phone_err) && empty($marks_err) && empty($password_err) && empty($c_pass_err) && empty($gender_err) && empty($course_err) && empty($dob_err) ) {
        $id = $_GET['id'];
        $sql = "UPDATE `student` SET `Name`='$name',`Email`='$email',`Number`='$phone',`Password`='$password',`Gender`='$gender ',`Marks`='$marks',`Course`='$course ',`Date_Of_Birth`='$dob' WHERE `Id` = '$id'";
        echo $name;
